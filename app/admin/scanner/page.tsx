    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white shadow-sm px-6 py-4 flex justify-between items-center sticky top-0 z-10">
        <div className="flex items-center gap-3">
          <Image src="/logo2.jpg" alt="Logo" width={45} height={45} className="object-contain mix-blend-multiply" />
          <div>
      ...
      <div className="max-w-5xl mx-auto p-4 sm:p-6">
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          <div className="bg-white rounded-2xl p-5 shadow text-center">
            ...
            <p className="text-5xl font-bold text-[#C41E3A]">{stats.total}</p>
          </div>
          <div className="bg-white rounded-2xl p-5 shadow text-center">
            <p className="text-gray-400 text-sm mb-2">Inside Hall</p>
            <p className="text-5xl font-bold text-green-600">{stats.entered}</p>
          </div>
          <div className="bg-white rounded-2xl p-5 shadow text-center">
            <p className="text-gray-400 text-sm mb-2">Outside</p>
            <p className="text-5xl font-bold text-gray-400">{stats.not_entered}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 items-start">
          ...
          <div className="bg-white rounded-2xl shadow p-6 sm:col-span-1">
            ...
            <div className="flex gap-3">
              {!scanning ? (
                ...
                <button onClick={stopScanner} className="flex-1 bg-gray-500 text-white py-3 rounded-xl font-medium hover:bg-gray-600 transition">
                  Stop Scanning
                </button>
              )}
              {result && (
                <button onClick={() => { setResult(null); setMessage('') }} className="flex-1 bg-gray-200 text-gray-700 py-3 rounded-xl font-medium hover:bg-gray-300 transition">
                  Scan Again
                </button>
              )}
            </div>
          ...
          <div className="space-y-4">
            {message && (
              <div className={`rounded-2xl p-5 text-center font-medium text-base ${
            ...
            {result && (
              <div className="bg-white rounded-2xl shadow p-6 space-y-4">
                <h3 className="font-bold text-lg border-b pb-3">Student Details</h3>
                {[
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className={`px-3 py-1 rounded-full text-sm font-medium ${result.status === 'ENTERED' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'}`}>
